...
action :create_volume do
  converge_by('Adding cloud block storage volume') do
    ...
    fail 'Cannot create a volume with a specific id (CBS chooses volume ids)' if @new_resource.volume_id
...
def locate_server
  server_id = `xenstore-read name`.sub('instance-', '').strip # ~FC048
  Chef::Log.info("Node matched to compute server #{server_id}")
  server = compute.servers.get(server_id)
  fail 'unable to locate server in compute API' if server.nil?
  server
end
...
def match_existing_volume
  ...
  unless @new_resource.volume_id.nil?
    ...
    fail "volume with id #{@new_resource.volume_id} could not be found" if volume.nil?
    @current_resource.exists = true
    @current_resource.volume_id(volume.id)
  else
    cbs.volumes.each do |volume|
      next unless @current_resource.name == volume.display_name
      @current_resource.exists = true
      @current_resource.volume_id(volume.id)
      break
    end
...
def attach_volume
  ...
  unless volume.attachments.empty?
    fail "Volume with id #{volume.id} exists but is attached to a different cloud server"
  end
...
def delete_volume
  ...
  unless volume.attachments.empty?
    fail "Volume #{volume.id} can not be deleted because it has active attachments"
  end
